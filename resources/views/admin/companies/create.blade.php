            ...
            <div class="dash_content_app_header_actions">
                <nav class="dash_content_app_breadcrumb">
                    <ul>
                        ...
                        <li class="separator icon-angle-right icon-notext"></li>
                        <li><a class="text-orange">Nova Empresa</a></li>
                    </ul>
                </nav>
            </div>
        </header>
                ...
                <form class="app_form" action="{{ route('admin.companies.store') }}" method="post">
                    ...
                    <label class="label">
                        ...
                        <select name="user" class="select2">
                            <option value="" selected>Selecione o responsável legal</option>
                            @foreach($users as $user)
                                @if(!empty($selected))
                                    <option
                                        value="{{ $user->id }}" {{ ($user->id === $selected->id ? 'selected' : '') }}>{{ $user->name }} ({{ $user->document }})</option>
                                @else
